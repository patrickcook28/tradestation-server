...
class PositionsStreamManager {
  ...
  async ensureUpstream(userId, accountId, paperTrading) {
    const key = PositionsStreamManager.makeKey(userId, accountId, paperTrading);
    const entry = this.keyToConnection.get(key);
    if (entry && entry.upstream) {
      console.log(`[Positions] Reusing upstream for key=${key}. Subscribers=${entry.subscribers.size}`);
      return entry;
    }

    const inFlight = this.pendingOpens.get(key);
    if (inFlight) {
      console.log(`[Positions] Awaiting pending upstream open for key=${key} ...`);
      try { await inFlight; } catch (_) {}
      const after = this.keyToConnection.get(key);
      if (after && after.upstream) {
        console.log(`[Positions] Reusing just-opened upstream for key=${key}.`);
        return after;
      }
    }
    ...
    const path = `/brokerage/stream/accounts/${accountId}/positions`;
    const url = buildUrl(!!paperTrading, path);

    let upstream;
    try {
      console.log(`[Positions] Opening upstream for key=${key} url=${url}`);
      ...
      if (!upstream.ok && upstream.status === 401) {
        try {
          ...
          console.log(`[Positions] Retrying upstream open after token refresh for key=${key}`);
          upstream = await fetch(url, {
            method: 'GET',
    ...
    const cleanup = () => {
      console.log(`[Positions] Upstream closing for key=${key}. Closing ${state.subscribers.size} subscriber(s).`);
      for (const res of state.subscribers) { try { res.end(); } catch (_) {} }
      state.subscribers.clear();
      this.keyToConnection.delete(key);
      console.log(`[Positions] Active upstreams: ${this.keyToConnection.size}`);
    };
  ...
  async addSubscriber(userId, accountId, paperTrading, res) {
    ...
    if (state.subscribers.size > 0) {
      console.log(`[Positions] Replacing existing subscriber(s) for key=${key}.`);
      for (const existing of state.subscribers) { try { existing.end(); } catch (_) {} }
      state.subscribers.clear();
    }
    state.subscribers.add(res);
    console.log(`[Positions] Subscriber added for key=${key}. Subscribers=${state.subscribers.size}. Active upstreams=${this.keyToConnection.size}`);
    ...
    const onClose = () => {
      ...
      if (state.subscribers.size === 0) {
        ...
        this.keyToConnection.delete(key);
        console.log(`[Positions] Subscriber closed for key=${key}. Active upstreams=${this.keyToConnection.size}`);
      }
    };
...
module.exports = new PositionsStreamManager();
